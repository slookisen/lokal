...
class ConversationService {
    ...
    startConversation(opts) {
      ...
      VALUES (?, ?, ?, 'open', ?, ?, ?, ?)
    `).run(id, opts.buyerAgentId || null, opts.sellerAgentId, opts.queryText || null, opts.taskId || null, now, now);
        ...
        this.addMessage({
            ...
            content: systemMsg,
            messageType: "info",
            metadata: { sellerAgentId: opts.sellerAgentId, queryText: opts.queryText },
        });
        // Log the interaction
        interaction_logger_1.interactionLogger.log("message", {
            ...
            metadata: { conversationId: id, type: "conversation_started" },
        });
