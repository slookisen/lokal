...
exports.visionProviders = exports.VisionProviderManager = exports.OpenAIVisionProvider = exports.ClaudeVisionProvider = exports.SmartMockProvider = exports.NORWEGIAN_PRODUCE = void 0;
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
// ─── Norwegian Produce Data (shared by all providers) ────────────
// This is passed as context to AI models and used by the mock.
exports.NORWEGIAN_PRODUCE = {
    tomater: {
        category: "vegetables",
...
class SmartMockProvider {
    ...
    async analyze(request) {
        ...
        const detections = [];
        const produceKeys = Object.keys(exports.NORWEGIAN_PRODUCE);
        // Strategy 1: Parse filename for hints
        const filenameHints = request.imagePaths
            .map((p) => path_1.default.basename(p).toLowerCase())
            .join(" ");
        ...
        const count = Math.min(baseCount + Math.floor(Math.random() * 2), produceKeys.length);
        ...
        for (const key of produceKeys) {
            ...
            const produce = exports.NORWEGIAN_PRODUCE[key];
...
exports.SmartMockProvider = SmartMockProvider;
...
class ClaudeVisionProvider {
    ...
    async analyze(request) {
        if (!this.apiKey) {
            throw new Error("ANTHROPIC_API_KEY ikke satt. Legg til i .env for å aktivere Claude Vision.");
        }
        const start = Date.now();
        ...
        const produceContext = Object.entries(exports.NORWEGIAN_PRODUCE)
            .map(([key, info]) => `- ${key}: sorter=[${info.varieties.join(", ")}], enhet=${info.unit}, pris=${info.priceRange[0]}-${info.priceRange[1]} kr`)
            .join("\n");
...
exports.ClaudeVisionProvider = ClaudeVisionProvider;
...
class OpenAIVisionProvider {
    ...
    async analyze(request) {
        if (!this.apiKey) {
            throw new Error("OPENAI_API_KEY ikke satt. Legg til i .env for å aktivere OpenAI Vision.");
        }
        const start = Date.now();
...
exports.VisionProviderManager = VisionProviderManager;
exports.visionProviders = new VisionProviderManager();
