...
class ReservationService {
    ...
    create(input) {
        ...
        if (!producer)
            throw new Error("Produsent ikke funnet");
        ...
        for (const reqItem of input.items) {
            const product = store_1.store.getProduct(reqItem.productId);
            if (!product)
                throw new Error(`Produkt ${reqItem.productId} ikke funnet`);
            const inventory = store_1.store.getInventoryForProduct(reqItem.productId);
            const available = inventory.find((i) => i.producerId === input.producerId && i.status === "available");
            if (!available) {
                throw new Error(`${product.name} er ikke tilgjengelig akkurat nå`);
            }
            if (available.quantityAvailable < reqItem.quantity) {
                throw new Error(`Kun ${available.quantityAvailable} ${available.unit} ${product.name} tilgjengelig (du ba om ${reqItem.quantity})`);
            }
            ...
            const chainPrice = store_1.store.getCheapestChainPrice(normalized, product.isOrganic);
            if (chainPrice) {
        ...
        const reservation = {
            ...
            savingsLabel: savings && savings > 0
     ...
     */
    confirm(reservationId, producerNote) {
        ...
        for (const item of res.items) {
            const inventory = store_1.store.getInventoryForProduct(item.productId);
            ...
            if (entry) {
                store_1.store.updateInventory({
                    ...
                    status: entry.quantityAvailable - item.quantity <= 0
                        ? "sold-out"
     ...
     */
    markReady(reservationId, producerNote) {
        const res = reservations.get(reservationId);
        if (!res)
            throw new Error("Reservasjon ikke funnet");
    ...
    complete(reservationId) {
        ...
        const producer = store_1.store.getProducer(res.producerId);
        if (producer) {
            const newTotal = producer.totalTransactions + 1;
            store_1.store.updateProducer(res.producerId, {
