...
exports.seedExpansionV4 = seedExpansionV4;
...
function seedExpansionV4() {
    const existing = marketplace_registry_1.marketplaceRegistry.getActiveAgents();
    const hasV4 = existing.some((a) => a.name === "Reindyrka Økologisk Dagligvare" || a.name === "REKO-ringen Gjøvik");
    if (hasV4) {
    ...
    // SUMMARY
    // ════════════════════════════════════════════════════════════
    const allAgents = marketplace_registry_1.marketplaceRegistry.getActiveAgents();
    const cities = [...new Set(allAgents.map((a) => a.city).filter(Boolean))];
    const producers = allAgents.filter((a) => a.role === "producer");
    console.log(`\n   ✅ Expansion v4 loaded:`);
...
//# sourceMappingURL=seed-expansion-v4.js.map