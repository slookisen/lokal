...
router.post("/tag-owner", (req, res) => {
    try {
        ...
        if (userAgentHashes.length > 0) {
            ...
            if (dryRun) {
                const count = db.prepare(`SELECT COUNT(*) as count FROM analytics_page_views WHERE user_agent_hash IN (${placeholders}) AND (is_owner IS NULL OR is_owner = 0)`).get(...userAgentHashes);
                results.pageViewsByUAHash = count.count;
            }
            else {
                const r = db.prepare(`UPDATE analytics_page_views SET is_owner = 1 WHERE user_agent_hash IN (${placeholders}) AND (is_owner IS NULL OR is_owner = 0)`).run(...userAgentHashes);
        ...
        if (ipHashes.length > 0) {
            ...
            for (const ip of ipHashes) {
                if (dryRun) {
                    const count = db.prepare(`SELECT COUNT(*) as count FROM analytics_page_views WHERE session_id LIKE ? AND (is_owner IS NULL OR is_owner = 0)`).get(ip + ":%");
                    pvByIp += count.count;
                }
                else {
                    const r = db.prepare(`UPDATE analytics_page_views SET is_owner = 1 WHERE session_id LIKE ? AND (is_owner IS NULL OR is_owner = 0)`).run(ip + ":%");
                    pvByIp += r.changes;
                }
            }
            ...
            if (dryRun) {
                const count = db.prepare(`SELECT COUNT(*) as count FROM analytics_queries WHERE client_ip_hash IN (${qPlaceholders}) AND (is_owner IS NULL OR is_owner = 0)`).get(...ipHashes);
                results.queriesByIP = count.count;
            }
            else {
                const r = db.prepare(`UPDATE analytics_queries SET is_owner = 1 WHERE client_ip_hash IN (${qPlaceholders}) AND (is_owner IS NULL OR is_owner = 0)`).run(...ipHashes);
 ...
 */
router.get("/traffic-classification", (req, res) => {
    const hours = Math.max(1, Math.min(720, parseInt(req.query.hours) || 24));
    try {
        const db = (0, init_1.getDb)();
        const cutoff = sqliteDatetime(new Date(Date.now() - hours * 60 * 60 * 1000));
    ...
    `).all(cutoff);
      ...
      GROUP BY session_id
    `).all(cutoff, ...scannerPaths.map(p => `%${p}%`));
        const scannerSessionIds = new Set(scannerHits.map((r) => r.session_id));
        ...
        for (const v of visitors) {
            ...
            if (isBot) {
                ...
                if (!botDetails[botName])
                    botDetails[botName] = { name: botName, views: 0 };
            ...
            classification[type].views += v.views;
            classification[type].sessions += 1;
        }
 ...
 */
router.post("/ops/clear-cache", (_req, res) => {
    try {
        ...
        if (analytics_service_1.analyticsService._summaryCache) {
            analytics_service_1.analyticsService._summaryCache.clear();
        }
...
router.post("/ops/prune", (req, res) => {
    try {
        ...
        const pvBefore = db.prepare("SELECT COUNT(*) as c FROM analytics_page_views").get().c;
        const qBefore = db.prepare("SELECT COUNT(*) as c FROM analytics_queries").get().c;
        const avBefore = db.prepare("SELECT COUNT(*) as c FROM analytics_agent_views").get().c;
        ...
        const pvAfter = db.prepare("SELECT COUNT(*) as c FROM analytics_page_views").get().c;
        const qAfter = db.prepare("SELECT COUNT(*) as c FROM analytics_queries").get().c;
        const avAfter = db.prepare("SELECT COUNT(*) as c FROM analytics_agent_views").get().c;
        // Reclaim disk space
 ...
 */
router.post("/ops/vacuum", (_req, res) => {
    try {
        const db = (0, init_1.getDb)();
        const fs = require("fs");
 ...
 */
router.get("/ops/diagnostics", (_req, res) => {
    try {
        const db = (0, init_1.getDb)();
        const fs = require("fs");
        const dbPath = process.env.DB_PATH || "./data/lokal.db";
        ...
        const qCount = db.prepare("SELECT COUNT(*) as c FROM analytics_queries").get().c;
        const avCount = db.prepare("SELECT COUNT(*) as c FROM analytics_agent_views").get().c;
        const agentCount = db.prepare("SELECT COUNT(*) as c FROM agents WHERE is_active = 1").get().c;
        // Oldest analytics record
        const oldestPv = db.prepare("SELECT MIN(created_at) as d FROM analytics_page_views").get()?.d;
        const oldestQ = db.prepare("SELECT MIN(created_at) as d FROM analytics_queries").get()?.d;
        // DB file size
        let dbSizeMb = 0;
        try {
