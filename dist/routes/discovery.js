    ...
    try {
        ...
        for (const [city, cityAgents] of sortedCities) {
            ...
            for (const a of cityAgents) {
                const cats = Array.isArray(a.categories) ? a.categories : (typeof a.categories === "string" ? JSON.parse(a.categories || "[]") : []);
                const k = knowledge_service_1.knowledgeService.getKnowledge(a.id);
                const parts = [`- **${a.name}**`];
                if (cats.length)
                    parts.push(`Kategorier: ${cats.join(", ")}`);
                if (k?.phone)
                    parts.push(`Tlf: ${k.phone}`);
                if (k?.website)
                    parts.push(`Web: ${k.website}`);
                if (k?.about)
                    parts.push(k.about.substring(0, 120));
...
// ═══════════════════════════════════════════════════════════════
router.get(["/terms", "/terms-of-service", "/tos", "/vilkar"], (_req, res) => {
    res.header("Content-Type", "text/html; charset=utf-8");
    res.header("Cache-Control", "public, max-age=3600");
    res.send(`<!DOCTYPE html>
<html lang="no">
