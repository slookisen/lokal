...
// ─── Page shell ─────────────────────────────────────────────
function shell(title, description, content, extra) {
...
router.get("/", (_req, res) => {
    try {
        ...
        const cityCards = topCities.map(([city, count], i) => `<a href="/${slugify(city)}" class="city-card">
        ...
        const featuredCards = featured.map((a) => producerCard(a)).join("");
    ...
    <section class="ai-sec">
      <div class="ai-banner">
        ...
        res.send(shell("Rett fra Bonden \u2014 Finn lokalprodusert mat i Norge", `S\u00f8k blant ${totalAgents} lokale matprodusenter i Norge. G\u00e5rder, markeder, g\u00e5rdsbutikker med kontaktinfo og \u00e5pningstider.`, content, { canonical: BASE_URL, jsonLd, extraCss: LANDING_CSS }));
    }
    catch (err) {
...
router.get("/om", (_req, res) => {
  ...
  <section class="om-sec">
    ...
    res.send(shell("Om Rett fra Bonden \u2014 V\u00e5r historie", "Rett fra Bonden gj\u00f8r lokale matprodusenter synlige for AI-assistenter. Les om v\u00e5r misjon og hvorfor vi bygger dette.", content, { canonical: `${BASE_URL}/om`, extraCss: OM_CSS }));
});
// ═══════════════════════════════════════════════════════════════
...
router.get("/:city", (req, res, next) => {
    ...
    try {
        ...
        if (cityAgents.length === 0) {
            return res.status(404).send(shell("Fant ingen produsenter", "Ingen produsenter funnet.", `<div class="sec" style="text-align:center;padding:80px 24px;">
    ...
    <section class="sec">
      <div class="city-grid">${producerCards}</div>
        ...
        res.send(shell(`Lokal mat i ${cityName} \u2014 ${cityAgents.length} produsenter`, `Finn ${cityAgents.length} lokale matprodusenter i ${cityName}. G\u00e5rder, markeder og g\u00e5rdsbutikker.`, content, { canonical: `${BASE_URL}/${citySlug}`, jsonLd: jsonLdItems, extraCss: CITY_CSS }));
    }
    catch (err) {
...
router.get("/produsent/:slug", (req, res) => {
    ...
    try {
        ...
        if (!agent) {
            return res.status(404).send(shell("Produsent ikke funnet", "Denne produsenten finnes ikke.", `<div class="sec" style="text-align:center;padding:80px 24px;">
        ...
        analytics_service_1.analyticsService.trackAgentView(agent.id, agent.name, cityName, "seo");
        ...
        const badges = [];
        if (agent.isVerified)
            badges.push(`<span class="badge badge-v">&#10003; Verifisert</span>`);
        const certs = k.certifications || [];
        if (certs.some((c) => c.toLowerCase().includes("kolog")))
            badges.push(`<span class="badge badge-o">&#127793; \u00d8kologisk</span>`);
        (agent.categories || []).slice(0, 3).forEach((c) => badges.push(`<span class="badge badge-c">${escapeHtml(formatCat(c))}</span>`));
        // Contact items
        const contactItems = [];
        if (k.address)
            contactItems.push(`<div class="ct-item"><div class="ct-icon">&#128205;</div><div><div class="ct-label">Adresse</div><div class="ct-val">${escapeHtml(k.address)}${k.postalCode ? `, ${escapeHtml(k.postalCode)}` : ""}</div></div></div>`);
        if (k.phone)
            contactItems.push(`<div class="ct-item"><div class="ct-icon">&#128222;</div><div><div class="ct-label">Telefon</div><div class="ct-val"><a href="tel:${k.phone.replace(/\s+/g, "")}">${escapeHtml(k.phone)}</a></div></div></div>`);
        if (k.email)
        ...
        const related = cityName
            ...
            }).sort((a, b) => (b.trustScore || 0) - (a.trustScore || 0)).slice(0, 4)
            : [];
        const relatedHtml = related.map((a) => {
            const trust = Math.round((a.trustScore || 0) * 100);
            const cats = (a.categories || []).slice(0, 2).map((c) => `<span class="tag" style="font-size:0.66rem;">${escapeHtml(formatCat(c))}</span>`).join("");
    ...
    <div class="pf-content">
      ...
      <div>
        ...
        res.send(shell(`${agent.name} \u2014 Lokal mat${cityName ? ` i ${cityName}` : ""}`, `${agent.name}${cityName ? ` i ${cityName}` : ""}. ${agent.description || "Lokalprodusert mat i Norge."}`, content, { canonical: `${BASE_URL}/produsent/${slug}`, jsonLd, extraCss: PROFILE_CSS }));
    }
    catch (err) {
