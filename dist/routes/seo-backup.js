...
// ─── Page shell ─────────────────────────────────────────────
function htmlShell(title, description, content, extra) {
...
router.get("/", (_req, res) => {
    try {
        ...
        // City cards
        const cityCards = topCities.map(([city, count]) => `<a href="/${slugify(city)}" class="city-card">
        ...
        const featuredCards = featured.map((a) => {
            const slug = slugify(a.name);
            const city = a.city || a.location?.city || "";
            const cats = (a.categories || []).slice(0, 3).map((c) => `<span class="tag">${escapeHtml(CATEGORY_MAP[c]?.label || c)}</span>`).join("");
        ...
        agents.forEach((a) => (a.categories || []).forEach((c) => uniqueCategories.add(c)));
        const jsonLd = {
    ...
    <div class="section">
      <div class="container">
        ...
        res.send(htmlShell("Rett fra Bonden — Finn lokalprodusert mat i Norge", `Sok blant ${totalCount} lokale matprodusenter i Norge. Garder, markeder, gaardsbutikker med kontaktinfo og apningstider.`, content, { canonical: BASE_URL, jsonLd }));
    }
    catch (err) {
...
router.get("/sok", (req, res) => {
    ...
    try {
        ...
        const resultCards = results.map((r) => {
            ...
            const city = a.city || a.location?.city || "";
            const slug = slugify(a.name);
            const cats = (a.categories || []).map((c) => `<span class="tag">${escapeHtml(CATEGORY_MAP[c]?.label || c)}</span>`).join("");
    ...
    <div class="container" style="padding-top:32px; padding-bottom:48px; min-height:60vh;">
      ...
      <div class="result-list">${resultCards || "<p style='color:var(--g500)'>Ingen resultater. Prov et bredere sok.</p>"}</div>
        ...
        res.send(htmlShell(`${q} — Rett fra Bonden`, `Sokresultater for "${q}" — finn lokale matprodusenter i Norge.`, content, { canonical: `${BASE_URL}/sok?q=${encodeURIComponent(q)}` }));
    }
    catch (err) {
...
router.get("/:city", (req, res, next) => {
    ...
    try {
        ...
        if (cityAgents.length === 0) {
            return res.status(404).send(htmlShell("Fant ingen produsenter", "Ingen produsenter funnet for denne byen.", `<div class="container" style="padding: 80px 0; text-align: center;">
        ...
        const jsonLdItems = cityAgents.slice(0, 50).map((a) => {
            ...
            if (k.address)
                item.address = { "@type": "PostalAddress", "streetAddress": k.address, "addressLocality": cityName, "addressCountry": "NO" };
            if (k.phone)
                item.telephone = k.phone;
            if (k.email)
                item.email = k.email;
            if (k.website)
                item.sameAs = k.website;
            if (a.location?.lat && a.location?.lng)
                item.geo = { "@type": "GeoCoordinates", "latitude": a.location.lat, "longitude": a.location.lng };
            if (a.categories?.length)
                item.keywords = a.categories.join(", ");
            return item;
        });
        const producerCards = cityAgents.map((a) => {
            const slug = slugify(a.name);
            const cats = (a.categories || []).slice(0, 3).map((c) => `<span class="tag">${escapeHtml(CATEGORY_MAP[c]?.label || c)}</span>`).join("");
            const trustPct = Math.round((a.trustScore || 0.5) * 100);
            return `<div class="prod-card">
    ...
    <div class="container" style="padding-top:16px; padding-bottom:48px;">
      ...
      <div class="prod-grid">${producerCards}</div>
        ...
        res.send(htmlShell(`Lokal mat i ${cityName} — ${cityAgents.length} produsenter | Rett fra Bonden`, `Finn ${cityAgents.length} lokale matprodusenter i ${cityName}. Garder, markeder og gaardsbutikker med kontaktinfo.`, content, { canonical: `${BASE_URL}/${citySlug}`, jsonLd: jsonLdItems }));
    }
    catch (err) {
...
router.get("/produsent/:slug", (req, res) => {
    ...
    try {
        const agents = marketplace_registry_1.marketplaceRegistry.getActiveAgents();
        const agent = agents.find((a) => slugify(a.name) === slug);
        if (!agent) {
            return res.status(404).send(htmlShell("Produsent ikke funnet", "Denne produsenten finnes ikke.", `<div class="container" style="padding: 80px 0; text-align: center;">
        ...
        const badges = [];
        if (agent.isVerified)
            badges.push(`<span class="badge badge-green">Verifisert</span>`);
        ...
        if (k.googleRating)
            statBlocks.push(`<div class="detail-stat"><div class="stat-val">${k.googleRating}</div><div class="stat-lbl">Google</div></div>`);
        // Contact sidebar
        const contactRows = [];
        if (k.address)
            contactRows.push(`<div class="contact-row">${ICON_HOME} <span>${escapeHtml(k.address)}${k.postalCode ? `, ${escapeHtml(k.postalCode)}` : ""}</span></div>`);
        if (k.phone)
            contactRows.push(`<div class="contact-row">${ICON_PHONE} <a href="tel:${k.phone.replace(/\s+/g, "")}">${escapeHtml(k.phone)}</a></div>`);
        if (k.email)
            contactRows.push(`<div class="contact-row">${ICON_MAIL} <a href="mailto:${k.email}">${escapeHtml(k.email)}</a></div>`);
        if (k.website)
            contactRows.push(`<div class="contact-row">${ICON_GLOBE} <a href="${escapeHtml(k.website)}" rel="noopener">${escapeHtml(k.website.replace(/^https?:\/\//, ""))}</a></div>`);
        const contactBtns = [];
        if (k.website)
            contactBtns.push(`<a href="${escapeHtml(k.website)}" class="btn-green" rel="noopener">Besok nettside</a>`);
        ...
        const dayNames = { mon: "Mandag", tue: "Tirsdag", wed: "Onsdag", thu: "Torsdag", fri: "Fredag", sat: "Lordag", sun: "Sondag" };
        ...
        const hoursHtml = k.openingHours?.length
          ...
          <div class="hours-grid">
            ${dayKeys.map((day, i) => {
                const h = k.openingHours.find((oh) => oh.day === day);
        ...
        const certsHtml = k.certifications?.length
          ...
          ${k.certifications.map((c) => `<span class="cert-badge">${escapeHtml(c)}</span>`).join(" ")}
        </div>`
            : "";
        // Payment & delivery
        const payHtml = k.paymentMethods?.length
            ? `<div class="detail-section"><h2>Betaling</h2><p style="color:var(--g700)">${k.paymentMethods.map((m) => escapeHtml(m)).join(", ")}</p></div>`
            : "";
        const deliveryHtml = k.deliveryOptions?.length
            ? `<div class="detail-section"><h2>Levering</h2><p style="color:var(--g700)">${k.deliveryOptions.map((d) => escapeHtml(d)).join(", ")}</p></div>`
            : "";
    ...
    <div class="container" style="padding-top:8px; padding-bottom:48px;">
      ...
      <div class="detail-layout">
        ...
        res.send(htmlShell(`${agent.name} — Lokal mat${cityName ? ` i ${cityName}` : ""} | Rett fra Bonden`, `${agent.name}${cityName ? ` i ${cityName}` : ""}. ${agent.description || "Lokalprodusert mat i Norge."}`, content, { canonical: `${BASE_URL}/produsent/${slug}`, jsonLd }));
    }
    catch (err) {
...
//# sourceMappingURL=seo-backup.js.map