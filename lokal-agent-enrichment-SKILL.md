...
**Agent selection for Tier 1:** Prioritize agents that ALREADY have basic data (name, address, about) but are MISSING googleRating, externalLinks, and openingHours. These are the agents where Tier 1 data adds the most visible value.

```python
# Selection logic for Tier 1:
# Score each agent by how many Tier 1 fields they're MISSING
tier1_fields = ['googleRating', 'externalLinks', 'openingHours']
# Also check contact gaps
contact_fields = ['phone', 'email', 'website']
# Priority = missing_tier1_count * 2 + missing_contact_count
...
curl -X PUT "https://rettfrabonden.com/api/marketplace/agents/{AGENT_ID}/knowledge" \
  ...
  -d '{
    ...
    "externalLinks": [
      ...
      {"label": "Nettbutikk", "url": "https://example.com/butikk", "type": "shop"}
    ],
    "dataSource": "auto",
    "autoSources": ["brreg.no", "google_maps", "1881.no", "facebook.com"]
  }'

# Step 3b: Update agent metadata (description, categories, tags)
curl -X PATCH "https://rettfrabonden.com/api/marketplace/agents/{AGENT_ID}" \
  -H "Content-Type: application/json" \
  -H "X-Admin-Key: $ADMIN_KEY" \
  -d '{
...
- Fields added per agent (broken down by: contact fields vs content fields)
