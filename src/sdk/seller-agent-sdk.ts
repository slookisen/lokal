...
interface Product {
  ...
  description?: string;
  pricePerUnit: number;
  unit: string;           // kg, stk, liter, pakke, etc.
...
interface SellerConfig {
  ...
  city: string;
  region?: string;
  lat?: number;              // optional override — auto-resolved from city if omitted
  lng?: number;              // optional override — auto-resolved from city if omitted
  description: string;
  categories: string[];
  tags?: string[];
  contactEmail?: string;     // recommended — used for registration
  phone?: string;
  website?: string;          // used as agent URL if provided
  address?: string;
  openingHours?: string;
  products: Product[];

  // Pre-existing agent credentials (skip registration if set)
  existingAgentId?: string;
  existingApiKey?: string;

  // Behavior config
  autoRespond?: boolean;        // auto-reply to matching queries (default: true)
  responseDelayMs?: number;     // simulate human delay (default: 1000)
  pollingIntervalMs?: number;   // how often to check for new conversations (default: 5000)
  maxConcurrentConversations?: number;  // limit active conversations (default: 10)

  // Custom response handler (optional — overrides auto-respond)
...
interface Message {
  ...
  content: string;
  messageType: 'text' | 'offer' | 'accept' | 'reject' | 'info';
  createdAt: string;
}
...
export class SellerAgent {
  ...
  private respondedConversations = new Set<string>();
  ...
  private async findExistingAgent(): Promise<void> {
    const res = await fetch(`${LOKAL_BASE}/api/marketplace/search?q=${encodeURIComponent(this.config.name)}`);
    const data = await res.json();
    const results = (data.results || []).map((r: any) => r.agent || r);
    const match = results.find((a: any) => a.name === this.config.name);
    if (match) {
  ...
  private async syncInventory(): Promise<void> {
    if (!this.agentId || !this.apiKey) return;
    ...
    const poll = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `${LOKAL_BASE}/api/conversations?sellerAgentId=${this.agentId}&status=open&limit=20`
        );
        ...
        for (const conv of conversations) {
          ...
          if (this.respondedConversations.has(conv.id)) continue;

          // Skip if at max concurrent
          if (this.activeConversations.size >= (this.config.maxConcurrentConversations || 10)) continue;
          ...
          const sellerMessages = (conversation.messages || []).filter(
            (m: Message) => m.senderRole === 'seller' && m.senderAgentId === this.agentId
          );
  ...
  private async pollViaTasks(): Promise<void> {
    ...
    try {
      ...
      if (data.result && Array.isArray(data.result)) {
        for (const task of data.result) {
          if (this.respondedConversations.has(task.id)) continue;
   ...
   */
  async respondToConversation(conversationId: string, message: string, type: string = 'text'): Promise<void> {
  ...
  private findMatchingProducts(query: string): Product[] {
    ...
    return this.config.products.filter(product => {
      ...
      if (a.organic && !b.organic) return -1;
      if (!a.organic && b.organic) return 1;
  ...
  private generateResponse(query: string, matches: Product[]): string {
    if (matches.length === 0) {
    ...
    const productList = matches.slice(0, 3).map(p => {
      ...
      if (p.available < 999) line += ` (${p.available} ${p.unit} tilgjengelig)`;
      if (p.organic) line += ' 🌿 Økologisk';
  ...
  private extractQuery(conversation: Conversation): string {
    const messages = conversation.messages || [];
    const buyerMsg = messages.find((m: Message) => m.senderRole === 'buyer');
    const systemMsg = messages.find((m: Message) => m.senderRole === 'system');
    return buyerMsg?.content || systemMsg?.content || '';
  }

  private async sendMessage(conversationId: string, content: string, messageType: string): Promise<void> {
    try {
