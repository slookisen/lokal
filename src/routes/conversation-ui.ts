...
router.get("/samtaler", (_req: Request, res: Response) => {
  try {
    ...
    if (conversations.length === 0) {
      ...
      listHtml = conversations.map(conv => {
        const buyerName = escapeHtml(conv.buyerAgentName || "Anonym kjøper");
        const sellerName = escapeHtml(conv.sellerAgentName || "Ukjent selger");
        const lastMsg = conv.messages.length > 0
          ? escapeHtml(conv.messages[conv.messages.length - 1].content).slice(0, 120)
          : "";
        const queryHtml = conv.queryText
        ...
        return `<a href="/samtale/${conv.id}" class="conv-item">
          ...
          <div class="conv-meta">
            ...
            <span class="conv-count">${conv.messages.length} meldinger</span>
          </div>
        </a>`;
      }).join("\n");
    }
