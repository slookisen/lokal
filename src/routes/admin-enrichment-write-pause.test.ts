  ...
  try {
    ...
    assertEq(
      ...
      "routes/admin-agents-internal-note-sweep.ts",
        ...
        // experiences-orgnr-from-name-kommune (dev-request 2026-09-14-
        // opplevagent-karantene-utgang-brreg-krav, Trinn B): a 20th route
        // gated by the SAME shared opplevelser.ts helper, plus its own test
        // file, which sets up pause state directly via the service to prove
        // the wiring (same convention as its sibling recheck-backfill's own
        // test file immediately above).
