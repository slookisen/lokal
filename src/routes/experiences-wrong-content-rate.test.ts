    ...
    try {
      ...
      // ═══════════════════════════════════════════════════════════════════
      {
        ...
        const insert = db.prepare(
          `INSERT INTO experiences (id, title, description, category, price_band, price_from, evidence_url, enrichment_state, content_source, updated_at)
           VALUES (@id, @title, @description, @category, @price_band, @price_from, @evidence_url, @enrichment_state, @content_source, @updated_at)`,
        );
        insert.run({ id: "e-eligible-1", title: "A", description: "d", category: "c", price_band: "standard", price_from: 100, evidence_url: "https://a.no", enrichment_state: "enriched", content_source: "provider_site", updated_at: "2026-08-01" });
        insert.run({ id: "e-eligible-2", title: "B", description: "d", category: "c", price_band: "standard", price_from: 100, evidence_url: "https://b.no", enrichment_state: "enriched", content_source: "provider_site", updated_at: "2026-08-02" });
        // Not eligible: wrong enrichment_state.
        insert.run({ id: "e-not-enriched", title: "C", description: "d", category: "c", price_band: "standard", price_from: 100, evidence_url: "https://c.no", enrichment_state: "matched", content_source: "provider_site", updated_at: "2026-08-01" });
        // Not eligible: wrong content_source.
        insert.run({ id: "e-not-provider-site", title: "D", description: "d", category: "c", price_band: "standard", price_from: 100, evidence_url: "https://d.no", enrichment_state: "enriched", content_source: "manual", updated_at: "2026-08-01" });
        // Not eligible: no evidence_url.
        insert.run({ id: "e-no-evidence", title: "E", description: "d", category: "c", price_band: "standard", price_from: 100, evidence_url: null, enrichment_state: "enriched", content_source: "provider_site", updated_at: "2026-08-01" });

        const sampled = sampleEnrichedExperiencesForHoldout(db as any, 10);
        assertEq(sampled.length, 2, "se-1: only the 2 genuinely-eligible rows are sampled");
        const sampledIds = sampled.map((r) => r.id).sort();
        assertEq(sampledIds, ["e-eligible-1", "e-eligible-2"], "se-2: exactly the eligible rows, ineligible ones excluded");
