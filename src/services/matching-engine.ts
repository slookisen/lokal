...
export class MatchingEngine {
  ...
  search(request: SearchRequest): SearchResult[] {
    const allInventory = store.getAvailableInventory();
    const results: SearchResult[] = [];

    for (const inventory of allInventory) {
      const product = store.getProduct(inventory.productId);
      const producer = store.getProducer(inventory.producerId);
      if (!product || !producer) continue;

      // ── Step 1: Hard Filters ──────────────────────────────
      if (!this.passesFilters(request, inventory, product, producer)) continue;
      ...
      const chainComparison = this.compareWithChains(
        product.name,
  ...
  private passesFilters(
    ...
    // Category filter
    if (request.category && product.category !== request.category) return false;
    ...
    // Organic filter
    if (request.mustBeOrganic && !product.isOrganic) return false;

    // Seasonal filter
    if (request.mustBeSeasonal && !product.isSeasonal) return false;

    // Text search (simple substring match for MVP)
    if (request.query) {
      const q = request.query.toLowerCase();
      const searchable =
        `${product.name} ${product.category} ${product.description || ""} ${producer.name}`.toLowerCase();
  ...
  private priceScore(inventory: InventoryEntry, product: Product): number {
    // Compare against chain prices if available
    const chainPrice = store.getCheapestChainPrice(
    ...
    if (chainPrice) {
      ...
      if (ratio <= 0.8) return 1.0; // 20%+ cheaper
      if (ratio <= 1.0) return 0.8; // cheaper or same
      if (ratio <= 1.2) return 0.5; // up to 20% more expensive
  ...
  private freshnessScore(inventory: InventoryEntry): number {
    if (!inventory.harvestedAt) return 0.5; // unknown freshness

    const hoursSinceHarvest =
      (Date.now() - new Date(inventory.harvestedAt).getTime()) /
      (1000 * 60 * 60);

    if (hoursSinceHarvest <= 4) return 1.0; // ultra fresh
    if (hoursSinceHarvest <= 12) return 0.9;
    if (hoursSinceHarvest <= 24) return 0.7;
    if (hoursSinceHarvest <= 48) return 0.5;
    return 0.3;
  }

  private distanceScore(distanceKm: number, maxDistanceKm: number): number {
    if (distanceKm <= 0.5) return 1.0;
    if (distanceKm <= 1) return 0.9;
    if (distanceKm >= maxDistanceKm) return 0.1;
  ...
  private compareWithChains(
    ...
    localPrice: number,
    isOrganic: boolean
  ): SearchResult["chainComparison"] | null {
    const normalized = this.normalizeProductName(productName);
    const cheapest = store.getCheapestChainPrice(normalized, isOrganic);

    if (!cheapest) return null;

    const diff = localPrice - cheapest.pricePerUnit;
    const percentDiff = Math.round(
      ((localPrice - cheapest.pricePerUnit) / cheapest.pricePerUnit) * 100
    );
  ...
  private generateMatchReasons(
    ...
    if (inventory.harvestedAt) {
      const hours =
        (Date.now() - new Date(inventory.harvestedAt).getTime()) /
        (1000 * 60 * 60);
      if (hours < 4) reasons.push("Plukket for under 4 timer siden");
      else if (hours < 24)
        reasons.push(`Plukket for ${Math.round(hours)} timer siden`);
    }

    // Organic
    if (product.isOrganic) reasons.push("Økologisk");

    // Seasonal
    if (product.isSeasonal) reasons.push("Sesongvare");

    // Trust
    if (producer.trustScore >= 0.8) reasons.push("Høy tillit");

    // Producer type
    if (producer.type === "farm") reasons.push("Direkte fra gården");
    if (producer.type === "garden") reasons.push("Fra lokal hage");
