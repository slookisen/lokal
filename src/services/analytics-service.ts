...
function parseUserAgent(ua: string): UAParseResult {
  ...
  // AI agent detection
  if (lower.includes("chatgpt")) return { isBot: true, clientType: "chatgpt", clientName: "ChatGPT" };
  if (lower.includes("claude")) return { isBot: true, clientType: "claude", clientName: "Claude" };
  if (lower.includes("gpt-4") || lower.includes("gpt-3")) return { isBot: true, clientType: "chatgpt", clientName: "GPT" };
  if (lower.includes("gemini")) return { isBot: true, clientType: "gemini", clientName: "Gemini" };
  if (lower.includes("bingbot") || lower.includes("googlebot")) return { isBot: true, clientType: "a2a-agent", botSource: "search_engine" };
