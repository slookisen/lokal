...
export function runCartServiceWishesTests(opts: { log?: boolean } = {}): TestSummary {
  ...
  try {
    ...
    // cart-service.ts) — set here so this file's pre-existing "eligible
    // producer's chosen offer becomes a real order" scenarios keep testing
    // the WISH-mirroring mechanics they were written for, unentangled from
    // that separate, orthogonal gate.
