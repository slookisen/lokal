...
// As the ecosystem grows, this list grows.
// Format: { name, registrationUrl, heartbeatUrl, type }

interface RegistryEntry {
  name: string;
  registrationUrl: string;
  heartbeatUrl?: string;
  type: "a2a" | "mcp" | "directory";
  enabled: boolean;
}
...
const KNOWN_REGISTRIES: RegistryEntry[] = [
  {
...
class DiscoveryService {
  ...
  async registerWithAllRegistries(): Promise<void> {
    const card = marketplaceRegistry.getRegistryCard(this.baseUrl);
  ...
  private async registerWithRegistry(registry: RegistryEntry, card: object): Promise<void> {
        ...
        try {
          await fetch(registry.heartbeatUrl!, {
            ...
            body: JSON.stringify({
              url: this.baseUrl,
              status: "active",
              stats: marketplaceRegistry.getStats(),
            }),
