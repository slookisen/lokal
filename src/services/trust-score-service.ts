...
class TrustScoreService {
  ...
  getBreakdown(agentId: string): {
    ...
    const tips: string[] = [];
    if (verification < 0.5) tips.push("Krev din agent for å øke tillitsscoren med opptil 30%");
    if (completeness < 0.5) tips.push("Legg til produkter, åpningstider og adresse for bedre synlighet");
    if (completeness < 0.8 && completeness >= 0.5) tips.push("Legg til bilder og sertifiseringer for enda bedre score");
    if (freshness < 0.5) tips.push("Oppdater profilen din jevnlig — aktive agenter rangeres høyere");
