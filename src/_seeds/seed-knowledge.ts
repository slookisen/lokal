...
import { knowledgeService, ProductInfo, OpeningHour } from "./services/knowledge-service";
...
const CATEGORY_PRODUCTS: Record<string, ProductInfo[]> = {
  vegetables: [
...
const OPENING_HOURS: Record<string, OpeningHour[]> = {
  reko: [
...
function detectType(name: string, desc: string): string {
  const n = name.toLowerCase();
  const d = desc.toLowerCase();
  if (n.includes("reko")) return "reko";
  if (n.includes("bondens marked") || n.includes("bondens butikk")) return "bondens_marked";
  if (n.includes("selvbetjen") || d.includes("self-service") || d.includes("selvbetjent") || d.includes("self-pick")) return "selvbetjening";
  if (n.includes("gårdsbutikk") || n.includes("gardsbutikk") || n.includes("gårdsutsalg") || n.includes("gardsutsalg") || n.includes("gårdsmat")) return "gardsbutikk";
  if (n.includes("ysteri") || n.includes("meieri")) return "gardsbutikk";
  if (n.includes("frukt") || n.includes("grønt") || n.includes("delikatesse") || n.includes("kolonial")) return "butikk";
  if (d.includes("farm shop") || d.includes("farmstand") || d.includes("farm") || d.includes("gård")) return "gardsbutikk";
  return "butikk";
}

// ─── Detect specialties from description ──────────────────
function detectSpecialties(name: string, desc: string): string[] {
  const specialties: string[] = [];
  const text = `${name} ${desc}`.toLowerCase();

  if (text.includes("organic") || text.includes("økologisk")) specialties.push("Økologisk produksjon");
  if (text.includes("award") || text.includes("prisvinn")) specialties.push("Prisvinnende produkter");
  if (text.includes("biodynamic") || text.includes("biodynamisk")) specialties.push("Biodynamisk");
  if (text.includes("arctic") || text.includes("arktisk")) specialties.push("Arktisk mat");
  if (text.includes("cheese") || text.includes("ost") || text.includes("ysteri")) specialties.push("Lokal osteproduksjon");
  if (text.includes("cider") || text.includes("sider")) specialties.push("Cider og saft");
  if (text.includes("smoked") || text.includes("røkt") || text.includes("røgeri")) specialties.push("Røkt mat");
  if (text.includes("honey") || text.includes("honning")) specialties.push("Lokal honning");
  if (text.includes("bakery") || text.includes("bakeri") || text.includes("brød")) specialties.push("Hjemmebakt brød");
  if (text.includes("highland cattle") || text.includes("highland")) specialties.push("Highland Cattle");
  if (text.includes("goat") || text.includes("geit")) specialties.push("Geiteprodukter");
...
function getPaymentMethods(type: string): string[] {
  switch (type) {
...
function getDeliveryOptions(type: string, desc: string): string[] {
...
// ─── Build about text ───────────────────────────────────
function buildAbout(name: string, desc: string, type: string, city: string): string {
...
export function seedKnowledge() {
  ...
  const existingCount = (db.prepare("SELECT COUNT(*) as c FROM agent_knowledge").get() as any).c;
  if (existingCount > 50) {
  ...
  const agents = db.prepare("SELECT id, name, description, city, categories, tags FROM agents WHERE is_active = 1").all() as any[];

  const enrichments: Array<{ agentId: string; data: any }> = [];

  for (const agent of agents) {
    const categories: string[] = agent.categories ? JSON.parse(agent.categories) : [];
    const tags: string[] = agent.tags ? JSON.parse(agent.tags) : [];
