  ...
  async ({ query, limit }) => {
    try {
      ...
      for (const r of data.results) {
        ...
        if (a.tags.length) text += `  Tags: ${a.tags.join(", ")}\n`;
        if (r.matchReasons.length) text += `  Match: ${r.matchReasons.join(" · ")}\n`;
        text += `\n`;
      }
...
server.tool(
  "lokal_discover",
  "Structured search in the Lokal food agent registry. Filter by food categories " +
  ...
  {
    categories: z.array(z.string()).optional().describe("Kategorier: vegetables, fruit, berries, dairy, eggs, meat, fish, bread, honey, herbs"),
    tags: z.array(z.string()).optional().describe("Tags: organic, seasonal, budget, local, fresh"),
    role: z.enum(["producer", "consumer", "logistics", "quality", "price-intel"]).default("producer").describe("Agentrolle"),
    lat: z.number().optional().describe("Breddegrad for avstandsfilter"),
    lng: z.number().optional().describe("Lengdegrad for avstandsfilter"),
    maxDistanceKm: z.number().optional().describe("Maks avstand i km"),
    limit: z.number().min(1).max(50).default(10).describe("Maks resultater"),
  },
  async ({ categories, tags, role, lat, lng, maxDistanceKm, limit }) => {
    try {
      const body: any = { role, limit };
...
server.tool(
  "lokal_register",
  "Register a new food producer, farm, shop, or cooperative as an agent in the Lokal marketplace. " +
  ...
  {
    name: z.string().describe("Agentens navn"),
    description: z.string().min(10).describe("Beskrivelse av hva agenten tilbyr"),
    provider: z.string().describe("Organisasjon bak agenten"),
    contactEmail: z.string().email().describe("Kontakt-epost"),
    url: z.string().url().describe("Agentens URL/endepunkt"),
    role: z.enum(["producer", "consumer", "logistics", "quality", "price-intel"]).describe("Agentrolle"),
    categories: z.array(z.string()).describe("Kategorier: vegetables, fruit, honey, etc."),
    tags: z.array(z.string()).describe("Tags: organic, local, fresh, etc."),
    city: z.string().default("Oslo").describe("By"),
    lat: z.number().optional().describe("Breddegrad"),
    lng: z.number().optional().describe("Lengdegrad"),
  },
  async ({ name, description, provider, contactEmail, url, role, categories, tags, city, lat, lng }) => {
    try {
      ...
      if (lat && lng) body.location = { lat, lng, city };
...
server.tool(
  "lokal_info",
  "Get Lokal platform statistics: total agents, active producers, cities covered, " +
  "health status, and A2A endpoint info. Use to verify the platform is running.",
  {},
  async () => {
    try {
...
server.tool(
  "lokal_jsonrpc",
  "Send a raw JSON-RPC 2.0 request to the Lokal A2A endpoint. " +
  "For advanced agent-to-agent communication using the A2A protocol standard.",
  {
    method: z.string().describe("JSON-RPC metode: message/send, tasks/get, tasks/list"),
    params: z.record(z.any()).describe("Metode-parametere som JSON objekt"),
  },
  async ({ method, params }) => {
    try {
...
function formatParsed(parsed: any): string {
  const parts: string[] = [];
  if (parsed.categories?.length) parts.push(`kategorier=[${parsed.categories.join(",")}]`);
  if (parsed.tags?.length) parts.push(`tags=[${parsed.tags.join(",")}]`);
  if (parsed.location) parts.push(`lokasjon=(${parsed.location.lat.toFixed(2)}, ${parsed.location.lng.toFixed(2)})`);
  if (parsed.maxDistanceKm) parts.push(`maks ${parsed.maxDistanceKm}km`);
