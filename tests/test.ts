...
async function _runCartTests() {
  // Wait for all other __setDbForTesting callers to finish before we pin cartDb
  // (serialised by the outer aggregator — this function is called AFTER it)
  ...
  // Restore global DB handle so any post-cart code sees a clean state
  try { initMod6.__setDbForTesting(null); } catch { /* tolerate if API differs */ }
...
(async () => {
  ...
  // Cart tests run LAST — after all other __setDbForTesting callers have settled,
  // so nothing can re-pin getDb() between our cartDb pin and the HTTP requests.
  try { await _runCartTests(); } catch { /* errors already pushed to failures */ }
