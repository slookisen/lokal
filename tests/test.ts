...
try {
  ...
  // Defer the DB swap until the M2 portal IIFE and PR-56/65/67/68/74/75
  // async blocks finish so we don't clobber their in-flight singleton-DB
  // state. PR-69-v2: added _m2Promise / _pr74Promise / _pr75Promise to
  // close the DB-singleton race surfaced in Supervisor Run 3.
  (async () => {
