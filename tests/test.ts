...
let _pr71Resolve: () => void = () => {};
const _pr71Promise: Promise<void> = new Promise<void>(r => { _pr71Resolve = r; });
...
// ── Behavioural: parseEventParticipants ──
{
...
// ── Behavioural: matchParticipantToAgent ──
{
...
// Same shape as the PR-56 scraper test. Wait for ALL prior async
// blocks that touch the DB singleton via __setDbForTesting() before
// stealing the global DB. Skipping any of these triggers race-failures
// in the test runner (observed in Runs 3/5/7/8 of iter-1/2/3).
(async () => {
  ...
  try { await _pr78Promise; } catch { /* counted */ }
  ...
  try {
    ...
    const failed = await runBmEventParticipantsScraper({ allUpcoming: true, fetcher: failingFetcher });
    assertEq(failed.events_skipped, 1, "pr71-pipeline: fetch failure counted as events_skipped");
    assertTrue(failed.errors.length >= 1, "pr71-pipeline: fetch failure pushes an error message");
  } catch (err: any) {
    failed++;
    failures.push(`✗ pr71 async block threw: ${err?.message || String(err)}`);
  } finally {
    _pr71Resolve();
  }
})();
