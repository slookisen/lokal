...
// ─── PR-96 (2026-06-02): Google-Places phone enrichment hardening ───
//
// Scenario D from the May-19 outreach-pool bottleneck analysis was
// already half-deployed (PR-82): Google Places address+phone are
// fetched when include_address_phone=true and written into provenance.
// PR-96 adds the missing safety so agents stuck in
// data_insufficient/review_required from single-source phone get
// promoted to pool_eligible without the risk of a Google-vs-producer
// phone mismatch silently lifting a disputed value to "agree".
//
// Tests pin five behaviours:
//   (1) Empty producer phone + Google phone → written to column AND
//       appended to provenance.phone.
//   (2) Producer phone matches Google (after digit-only normalisation,
//       ignoring +47/spaces) → column unchanged, provenance gets a
//       second source appended.
//   (3) Producer phone DISAGREES with Google → producer value retained,
//       WARNING logged, Google NOT appended to provenance.phone.
//   (4) Google returns no phone → provenance.phone unchanged.
//   (5) Idempotent re-call → no duplicate provenance entries.
//   (6) RFB_DISABLE_GOOGLE_PHONE_ENRICHMENT=1 → phone half disabled,
//       address half still works.
let _pr96Resolve: () => void = () => {};
const _pr96Promise = new Promise<void>((r) => { _pr96Resolve = r; });
console.log("\n── PR-96: Google-Places phone enrichment hardening ──");

(async () => {
  // Wait for earlier IIFEs that mutate the module-singleton DB so we don't
  // race against them swapping the DB out from under our seeds — AND so we
  // don't swap a DB out from under THEIR still-running HTTP servers. The
  // wait-list mirrors PR-95's (which is the most recent precedent) plus
  // _m2Promise / _pr24Promise / _pr56Promise / _pr67Promise / _pr68Promise
  // / _pr75Promise / _pr76Promise / _pr78Promise (all known to swap the
  // module-singleton DB via __setDbForTesting).
  try { await _m2Promise; } catch { /* failures recorded upstream */ }
  try { await _pr24Promise; } catch { /* failures recorded upstream */ }
  try { await _pr56Promise; } catch { /* failures recorded upstream */ }
  try { await _pr63Promise; } catch { /* failures recorded upstream */ }
  ...
  try { await _pr78Promise; } catch { /* failures recorded upstream */ }
  try { await _orchPr86Promise; } catch { /* failures recorded upstream */ }
  try { await _orchPr93Promise; } catch { /* failures recorded upstream */ }
  try { await _pr94Promise; } catch { /* failures recorded upstream */ }
  try { await _pr95Promise; } catch { /* failures recorded upstream */ }

  const fs96 = require("fs");
  const marketplaceSrc96 = fs96.readFileSync("src/routes/marketplace.ts", "utf-8");

  // ── pr96-1: source-text presence of the env-var off-switch ───────────
  assertTrue(
    /RFB_DISABLE_GOOGLE_PHONE_ENRICHMENT/.test(marketplaceSrc96),
    "pr96-1: marketplace.ts references RFB_DISABLE_GOOGLE_PHONE_ENRICHMENT env var"
  );
  assertTrue(
    /phoneEnrichmentDisabled\s*=\s*process\.env\.RFB_DISABLE_GOOGLE_PHONE_ENRICHMENT\s*===\s*"1"/.test(marketplaceSrc96),
    "pr96-1b: phoneEnrichmentDisabled checks env var === '1' (mirrors RFB_DISABLE_DEBIO_SYNC pattern)"
  );

  // ── pr96-2: source-text presence of the mismatch-warning logic ───────
  assertTrue(
    /\[pr96 google-phone-mismatch\]/.test(marketplaceSrc96),
    "pr96-2: marketplace.ts emits the [pr96 google-phone-mismatch] warning tag"
  );
  assertTrue(
    /appendPhoneSource/.test(marketplaceSrc96),
    "pr96-2b: marketplace.ts has appendPhoneSource decision variable"
  );

  // ── pr96-3: fieldMask varies with env-var ────────────────────────────
  // Three distinct masks: no-addr-phone / addr-only / addr+phone.
  assertTrue(
    marketplaceSrc96.includes("places.rating,places.userRatingCount,places.displayName,places.formattedAddress,places.internationalPhoneNumber"),
    "pr96-3a: addr+phone fieldMask string present"
  );
  assertTrue(
    marketplaceSrc96.includes('"places.rating,places.userRatingCount,places.displayName,places.formattedAddress"'),
    "pr96-3b: addr-only fieldMask string present (phone-disabled path)"
  );

  // ── pr96-4: mergeFieldProvenance dedupes phone google_places ────────
  // Idempotency check — repeated google_places phone records collapse
  // to a single entry (covers requirement #6 in the spec).
  {
    const adminKnowledgeMod96 = require("../src/routes/admin-knowledge");
    const merge96 = adminKnowledgeMod96.mergeFieldProvenance;
    const existing96 = {
      phone: [
        { value: "+4791193602", source_type: "homepage", fetched_at: "2026-05-20T00:00:00Z" },
      ],
    };
    const incoming96 = {
      phone: { sources: [{ source_type: "google_places", value: "+4791193602", fetched_at: "2026-06-02T00:00:00Z" }] },
    };
    const merged96a = merge96(existing96, incoming96);
    assertEq((merged96a.phone || []).length, 2, "pr96-4a: first merge adds google_places as 2nd phone source");
    const types96a = (merged96a.phone || []).map((r: any) => r.source_type).sort();
    assertEq(JSON.stringify(types96a), JSON.stringify(["google_places", "homepage"]),
      "pr96-4a-types: phone sources are homepage + google_places");

    // Second call with the SAME google_places value → dedupe.
    const merged96b = merge96(merged96a, incoming96);
    assertEq((merged96b.phone || []).length, 2, "pr96-4b: idempotent re-merge keeps phone at 2 sources");
  }

  // ── pr96-5: phone-mismatch normalisation matches validator ───────────
  // The marketplace handler's normalise-for-compare must agree with
  // cross-source-validator.normalizePhone semantics: +47, 0047, spaces,
  // dashes, parentheses all collapse to bare digits.
  {
    // Reach into the validator's normalizer indirectly by exercising
    // crossSourceAgreement with two records that differ only in
    // formatting — if the validator considers them equal (agreement),
    // then our handler's comparator must too.
    const { crossSourceAgreement } = require("../src/services/cross-source-validator");
    const r96c = crossSourceAgreement(
      {
        phone: [
          { value: "+47 911 93 602", source_type: "homepage", fetched_at: "2026-05-20T00:00:00Z" },
          { value: "91193602",       source_type: "google_places", fetched_at: "2026-06-02T00:00:00Z" },
        ],
      },
      "phone"
    );
    assertEq(r96c.verdict, "pool_eligible",
      "pr96-5a: validator treats +47 911 93 602 == 91193602 (digit-only equivalence)");
    assertEq(r96c.source_count, 2,
      "pr96-5b: cross-source phone count = 2 after google_places enrichment");

    // Mismatch case — different last digit → should NOT agree.
    const r96d = crossSourceAgreement(
      {
        phone: [
          { value: "+47 911 93 602", source_type: "homepage", fetched_at: "2026-05-20T00:00:00Z" },
          { value: "91193603",       source_type: "google_places", fetched_at: "2026-06-02T00:00:00Z" },
        ],
      },
      "phone"
    );
    // Two Tier-A sources, but they disagree → review_required, not pool_eligible.
    assertEq(r96d.verdict, "review_required",
      "pr96-5c: validator flags disagreeing +47 911 93 602 vs 91193603 as review_required");
  }

  // ── pr96-6: integration — stub fetch + hit the HTTP endpoint ─────────
  // This exercises the full path: route handler → mocked Google Places
  // response → DB write → provenance state. Covers tests 1,2,3,4,5
  // from the requirement spec.
  {
    const DatabaseCtor96 = require("better-sqlite3");
    const db96 = new DatabaseCtor96(":memory:");
    db96.exec(`
      CREATE TABLE agents (
        id TEXT PRIMARY KEY,
        name TEXT NOT NULL,
        ...
        is_active INTEGER DEFAULT 1,
        languages TEXT DEFAULT '["no"]',
        schema_version TEXT DEFAULT 'urn:a2a:1.0',
        agent_version INTEGER DEFAULT 1,
        debio_verified INTEGER DEFAULT 0,
        created_at TEXT DEFAULT (datetime('now')),
        last_seen_at TEXT,
        vertical_id TEXT DEFAULT 'rfb',
        description TEXT
      ...
      CREATE TABLE agent_knowledge (
        ...
        google_review_count INTEGER,
        tripadvisor_rating REAL,
        external_reviews TEXT DEFAULT '[]',
        images TEXT DEFAULT '[]',
        seasonality TEXT,
        delivery_radius INTEGER,
        min_order_value INTEGER,
        data_source TEXT DEFAULT 'auto',
        auto_sources TEXT DEFAULT '[]',
        last_enriched_at TEXT,
      ...
      CREATE TABLE agent_metrics (
        ...
        avg_response_text_length INTEGER,
        decline_rate REAL DEFAULT 0
      );
    `);
    const initMod96 = await import("../src/database/init");
    initMod96.__setDbForTesting(db96 as any);

    // Seed four agents covering the four scenarios.
    const seedAgent = (id: string, name: string, phone: string | null) => {
      db96.prepare("INSERT INTO agents (id, name, city, url, role) VALUES (?, ?, ?, ?, ?)")
        .run(id, name, "Oslo", `https://${id}.no`, "producer");
      db96.prepare("INSERT INTO agent_knowledge (agent_id, phone) VALUES (?, ?)")
        .run(id, phone);
    };
    seedAgent("pr96-empty",  "Empty Phone Gård",     null);
    seedAgent("pr96-match",  "Matching Phone Gård",  "+47 911 93 602");
    seedAgent("pr96-mismatch", "Mismatching Phone Gård", "+47 911 93 602");
    seedAgent("pr96-no-phone", "Google Has No Phone Gård", null);

    // Pin admin key + Google Places key.
    const PR96_KEY = "pr96-test-key";
    const prevAdminKey96 = process.env.ADMIN_KEY;
    const prevPlacesKey96 = process.env.GOOGLE_PLACES_API_KEY;
    const prevDisableKey96 = process.env.RFB_DISABLE_GOOGLE_PHONE_ENRICHMENT;
    process.env.ADMIN_KEY = PR96_KEY;
    process.env.GOOGLE_PLACES_API_KEY = "fake-test-key";
    delete process.env.RFB_DISABLE_GOOGLE_PHONE_ENRICHMENT;

    // Stub globalThis.fetch to return per-agent fixtures.
    const phoneByAgent: Record<string, string | undefined> = {
      "pr96-empty":    "+47 95 12 34 56",
      "pr96-match":    "+47 911 93 602",   // matches after normalisation
      "pr96-mismatch": "+47 911 93 999",   // disagrees in last 3 digits
      "pr96-no-phone": undefined,
    };
    const addrByAgent: Record<string, string> = {
      "pr96-empty":    "Storgata 1, Oslo",
      "pr96-match":    "Storgata 2, Oslo",
      "pr96-mismatch": "Storgata 3, Oslo",
      "pr96-no-phone": "Storgata 4, Oslo",
    };
    const prevFetch96 = (globalThis as any).fetch;
    let activeAgentId96: string | null = null;
    (globalThis as any).fetch = async (url: string, init?: any) => {
      if (typeof url === "string" && url.includes("places.googleapis.com")) {
        const body = init && typeof init.body === "string" ? init.body : "{}";
        let textQuery = "";
        try { textQuery = JSON.parse(body).textQuery || ""; } catch { /* */ }
        // Map textQuery back to agent name.
        let targetId: string | null = null;
        for (const id of Object.keys(addrByAgent)) {
          const row = db96.prepare("SELECT name FROM agents WHERE id = ?").get(id) as any;
          if (row && textQuery.includes(row.name)) { targetId = id; break; }
        }
        targetId = targetId || activeAgentId96;
        const phone = targetId ? phoneByAgent[targetId] : undefined;
        const addr = targetId ? addrByAgent[targetId] : "";
        const placeObj: any = {
          rating: 4.5,
          userRatingCount: 12,
          displayName: { text: targetId || "Test" },
          formattedAddress: addr,
        };
        if (phone) placeObj.internationalPhoneNumber = phone;
        return new Response(JSON.stringify({ places: [placeObj] }), {
          status: 200, headers: { "content-type": "application/json" },
        });
      }
      // Default fallback for any other fetch (e.g. URL probes from other code paths).
      return new Response("[]", { status: 200, headers: { "content-type": "application/json" } });
    };

    // Mount marketplace router on a fresh express app.
    const expressMod96 = (await import("express")).default;
    const marketplaceMod96 = await import("../src/routes/marketplace");
    const app96 = expressMod96();
    app96.use(expressMod96.json());
    app96.use(marketplaceMod96.default);

    const httpMod96 = await import("http");
    const server96 = httpMod96.createServer(app96);
    await new Promise<void>((resolve) => server96.listen(0, "127.0.0.1", () => resolve()));
    const addr96 = server96.address();
    const port96 = typeof addr96 === "object" && addr96 ? addr96.port : 0;

    function callBatch96(agentIds: string[], opts: { include_address_phone?: boolean } = {}): Promise<{ status: number; body: any }> {
      process.env.ADMIN_KEY = PR96_KEY;
      const payload = JSON.stringify({ agentIds, ...opts });
      return new Promise((resolve, reject) => {
        const r = httpMod96.request(
          { method: "POST", host: "127.0.0.1", port: port96, path: "/admin/google-rating-batch",
            headers: { "content-type": "application/json", "content-length": Buffer.byteLength(payload), "x-admin-key": PR96_KEY } },
          (resp) => {
            const chunks: Buffer[] = [];
            resp.on("data", (c) => chunks.push(c as Buffer));
            resp.on("end", () => {
    ...
    try {
      // ─── Test 1: empty producer phone → google fills it ──────────
      activeAgentId96 = "pr96-empty";
      const r1 = await callBatch96(["pr96-empty"], { include_address_phone: true });
      assertEq(r1.status, 200, "pr96-6.1-status: HTTP 200 on enrich-empty");
      const row1 = db96.prepare("SELECT phone, field_provenance FROM agent_knowledge WHERE agent_id = 'pr96-empty'").get() as any;
      assertEq(row1.phone, "+4795123456", "pr96-6.1a: empty phone now filled with Google value (normalised)");
      const prov1 = JSON.parse(row1.field_provenance);
      assertTrue(Array.isArray(prov1.phone) && prov1.phone.length === 1 && prov1.phone[0].source_type === "google_places",
        "pr96-6.1b: phone provenance has 1 entry (google_places)");

      // ─── Test 2: producer phone matches Google → append source ───
      activeAgentId96 = "pr96-match";
      // Seed homepage provenance for the matching agent first (simulates
      // prior enrichment cycle adding the homepage source).
      const seedProv = JSON.stringify({
        phone: [{ value: "+4791193602", source_type: "homepage", fetched_at: "2026-05-20T00:00:00Z" }],
      });
      db96.prepare("UPDATE agent_knowledge SET field_provenance = ? WHERE agent_id = 'pr96-match'").run(seedProv);

      const r2 = await callBatch96(["pr96-match"], { include_address_phone: true });
      assertEq(r2.status, 200, "pr96-6.2-status: HTTP 200 on enrich-match");
      const row2 = db96.prepare("SELECT phone, field_provenance FROM agent_knowledge WHERE agent_id = 'pr96-match'").get() as any;
      assertEq(row2.phone, "+47 911 93 602", "pr96-6.2a: producer phone untouched on match");
      const prov2 = JSON.parse(row2.field_provenance);
      assertEq(prov2.phone.length, 2, "pr96-6.2b: phone provenance has 2 sources after matching enrichment");
      const sourceTypes2 = prov2.phone.map((r: any) => r.source_type).sort();
      assertEq(JSON.stringify(sourceTypes2), JSON.stringify(["google_places", "homepage"]),
        "pr96-6.2c: phone sources are homepage + google_places");

      // ─── Test 3: producer phone DISAGREES with Google → no append ─
      activeAgentId96 = "pr96-mismatch";
      const seedProvMismatch = JSON.stringify({
        phone: [{ value: "+4791193602", source_type: "homepage", fetched_at: "2026-05-20T00:00:00Z" }],
      });
      db96.prepare("UPDATE agent_knowledge SET field_provenance = ? WHERE agent_id = 'pr96-mismatch'").run(seedProvMismatch);

      // Capture warn output to assert the mismatch log fired.
      const origWarn96 = console.warn;
      const warnLines96: string[] = [];
      console.warn = (...args: any[]) => { warnLines96.push(args.map(String).join(" ")); };
      try {
        const r3 = await callBatch96(["pr96-mismatch"], { include_address_phone: true });
        assertEq(r3.status, 200, "pr96-6.3-status: HTTP 200 on enrich-mismatch");
      } finally {
        console.warn = origWarn96;
      }
      const row3 = db96.prepare("SELECT phone, field_provenance FROM agent_knowledge WHERE agent_id = 'pr96-mismatch'").get() as any;
      assertEq(row3.phone, "+47 911 93 602", "pr96-6.3a: producer phone retained on mismatch");
      const prov3 = JSON.parse(row3.field_provenance);
      assertEq(prov3.phone.length, 1, "pr96-6.3b: phone provenance UNCHANGED on mismatch (still 1 source)");
      assertEq(prov3.phone[0].source_type, "homepage", "pr96-6.3c: only homepage source remains after mismatch rejection");
      assertTrue(warnLines96.some(l => l.includes("[pr96 google-phone-mismatch]")),
        "pr96-6.3d: mismatch warning logged");

      // ─── Test 4: Google returns no phone → no provenance update ──
      activeAgentId96 = "pr96-no-phone";
      const r4 = await callBatch96(["pr96-no-phone"], { include_address_phone: true });
      assertEq(r4.status, 200, "pr96-6.4-status: HTTP 200 on no-phone enrichment");
      const row4 = db96.prepare("SELECT phone, field_provenance FROM agent_knowledge WHERE agent_id = 'pr96-no-phone'").get() as any;
      assertEq(row4.phone, null, "pr96-6.4a: phone column untouched (null) when Google had no phone");
      const prov4 = JSON.parse(row4.field_provenance);
      assertTrue(!prov4.phone || prov4.phone.length === 0,
        "pr96-6.4b: phone provenance not set when Google had no phone");
      // Address still got written (provenance for address fired).
      assertTrue(Array.isArray(prov4.address) && prov4.address.length === 1,
        "pr96-6.4c: address provenance still recorded on no-phone path");

      // ─── Test 5: idempotent re-call ─────────────────────────────
      activeAgentId96 = "pr96-empty";
      const r5 = await callBatch96(["pr96-empty"], { include_address_phone: true });
      assertEq(r5.status, 200, "pr96-6.5-status: HTTP 200 on idempotent re-call");
      const row5 = db96.prepare("SELECT phone, field_provenance FROM agent_knowledge WHERE agent_id = 'pr96-empty'").get() as any;
      const prov5 = JSON.parse(row5.field_provenance);
      assertEq(prov5.phone.length, 1, "pr96-6.5a: re-call doesn't duplicate phone provenance entry");
      const gpPhoneEntries = prov5.phone.filter((r: any) => r.source_type === "google_places");
      assertEq(gpPhoneEntries.length, 1, "pr96-6.5b: exactly one google_places phone entry after re-call");

      // ─── Test 6: RFB_DISABLE_GOOGLE_PHONE_ENRICHMENT=1 ─────────
      // Wipe a fresh agent and verify phone path is skipped while
      // address path still fires.
      seedAgent("pr96-killswitch", "Killswitch Gård", null);
      process.env.RFB_DISABLE_GOOGLE_PHONE_ENRICHMENT = "1";
      phoneByAgent["pr96-killswitch"] = "+47 95 99 99 99";
      addrByAgent["pr96-killswitch"]  = "Killveien 99, Oslo";
      activeAgentId96 = "pr96-killswitch";
      const r6 = await callBatch96(["pr96-killswitch"], { include_address_phone: true });
      assertEq(r6.status, 200, "pr96-6.6-status: HTTP 200 with kill-switch on");
      const row6 = db96.prepare("SELECT phone, address, field_provenance FROM agent_knowledge WHERE agent_id = 'pr96-killswitch'").get() as any;
      assertEq(row6.phone, null, "pr96-6.6a: kill-switch on → phone column NOT written");
      assertEq(row6.address, "Killveien 99, Oslo", "pr96-6.6b: kill-switch on → address still written");
      const prov6 = JSON.parse(row6.field_provenance);
      assertTrue(!prov6.phone || prov6.phone.length === 0,
        "pr96-6.6c: kill-switch on → phone provenance NOT recorded");
      assertTrue(Array.isArray(prov6.address) && prov6.address.length === 1,
        "pr96-6.6d: kill-switch on → address provenance still recorded");
      // Per-agent result echoes phoneMismatch/phoneProvenanceAppended fields.
      const result6 = r6.body.data.results[0];
      assertEq(result6.phoneWritten, false, "pr96-6.6e: kill-switch result.phoneWritten=false");
      assertEq(result6.phoneProvenanceAppended, false,
        "pr96-6.6f: kill-switch result.phoneProvenanceAppended=false");

      // Restore env.
      if (prevDisableKey96 === undefined) delete process.env.RFB_DISABLE_GOOGLE_PHONE_ENRICHMENT;
      else process.env.RFB_DISABLE_GOOGLE_PHONE_ENRICHMENT = prevDisableKey96;
    } finally {
      await new Promise<void>((resolve) => server96.close(() => resolve()));
      (globalThis as any).fetch = prevFetch96;
      if (prevAdminKey96 === undefined) delete process.env.ADMIN_KEY;
      else process.env.ADMIN_KEY = prevAdminKey96;
      if (prevPlacesKey96 === undefined) delete process.env.GOOGLE_PLACES_API_KEY;
      else process.env.GOOGLE_PLACES_API_KEY = prevPlacesKey96;
      db96.close();
    }
  }
})().then(
  ...
  (err: any) => {
    failed++;
    failures.push(`✗ pr96 async block threw: ${err?.message || String(err)}`);
    _pr96Resolve();
  }
);
...
(async () => {
  ...
  try { await _pr96Promise; } catch { /* errors already pushed to failures */ }
