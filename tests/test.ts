...
// ── Behavioural tail: fetch-stubbed end-to-end. Runs AFTER PR-65/PR-66
// behavioural blocks so the global DB singleton isn't being thrashed.
(async () => {
  try {
    // PR-70 v2: explicit await chain to avoid DB-singleton race with M2 + earlier PRs
    try { await _m2Promise; } catch { /* m2 owns its failures */ }
