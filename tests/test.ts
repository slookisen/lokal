...
// PR-79 (2026-05-17): behavioural tail is wrapped in withTestDbMutex
// so the DB pin/unpin is serialized with other mutex-holding blocks.
// ════════════════════════════════════════════════════════════════════
console.log("\n── PR-70: Debio cross-check via finnoko (primary) ──");

// ── Source-presence (sync — no DB, safe at top-level) ───────────────
{
  const fs = require("fs");
...
// ── Behavioural tail: fetch-stubbed end-to-end via withTestDbMutex.
//    PR-79 mutex serializes the DB pin/unpin against every other
//    block that calls __setDbForTesting, so we no longer need to
//    await each upstream pr-N promise individually.
const _pr70Promise: Promise<void> = withTestDbMutex("pr-70", async () => {
  ...
  let callCount = 0;
  const okStub = async (_url: string): Promise<any> => {
    callCount++;
    return {
      ok: true,
      status: 200,
  ...
  failed++;
  failures.push(`✗ pr-70 behavioural: ${err instanceof Error ? err.message : String(err)}`);
});
