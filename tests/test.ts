...
// PR-79: serialized via the test-DB mutex (replaces the IIFE-await chain
// used by v1–v3). The mutex auto-clears __setDbForTesting on exit, so
// the (A)/(B)/(C) sections that don't pin a DB are still safe; the (D)
// section that does pin a DB has its cleanup handled by the mutex.
// ════════════════════════════════════════════════════════════════════
console.log("\n── PR-69: Hanen yield-lift (extractExternalWebsite + fylke_dual_corroboration) ──");

let _pr69Promise: Promise<unknown> = Promise.resolve();
...
_pr69Promise = withTestDbMutex("pr-69", async () => {
  try {
    ...
    // C1. Dice << 0.75 with dual signals → REJECT.
    const mC1 = {
    ...
    // C2. Dice >= 0.75 + dual-corroboration → ACCEPT.
    ...
    // PR-79: serialized via the surrounding withTestDbMutex; no need to await
    // PR-56/65/67/68 promises manually — the mutex queue already orders us
    // after PR-68 (which is enqueued before us). Cleanup of __setDbForTesting
    // is handled automatically by withTestDbMutex on exit.
    ...
    // PR-79: __setDbForTesting cleanup handled by withTestDbMutex on exit.
  } catch (err: any) {
    failed++;
