...
// DB-backed behavioural block — PR-79: serialized via withTestDbMutex.
const _pr77Promise = withTestDbMutex("pr-77", async () => {
  const Database = require("better-sqlite3");
  const initMod = require("../src/database/init");
  const db = new Database(":memory:");
  db.exec(`
  ...
  `);
  // Tree: national → BM Agder lokallag → {Arendal, Kristiansand venues}
  //                                    → BM Oslo lokallag → Birkelunden venue
  ...
  // Producers (HIGH-matching the fixture names).
  ...
  // Three future events: one per venue. Eiker attends ALL three (cross-lokallag).
  // Sætre attends Arendal + Kristiansand (same-lokallag). Grini attends only Birkelunden.
  ...
  // Expected rows:
  //   Eiker: venue {ven-arendal,ven-krs,ven-birk} + lokallag {lok-agder,lok-oslo} = 5
  //   Sætre: venue {ven-arendal,ven-krs} + lokallag {lok-agder}                   = 3
  //   Grini: venue {ven-birk} + lokallag {lok-oslo}                                = 2
  //   Total: 10.
  ...
  // ── [T4] evidence_json includes event_id + cross-links.
  ...
  // ── [T5] Status transitions correctly.
  ...
  failed++;
  failures.push(`✗ pr77 mutex block threw: ${err instanceof Error ? err.message : String(err)}`);
});
