...
24. [Phase 24: Marketplace Transactions (Cart MVP), Search-Enrich Pipeline, Outreach Suppression Gate & Verifier Industrialization](#phase-24)
25. [Phase 25: Structured Data Enrichment, Brreg Verification Gateway & Gaardssalg Industrialization](#phase-25)
26. [Phase 26: Loop Dispatch Reliability, Salgskanal Auto-Matcher & Brreg Registration Wiring](#phase-26)
27. [Appendix A: Tech Stack Reference](#appendix-a)
28. [Appendix B: Deployment Checklist](#appendix-b)
29. [Appendix C: Gotchas & Lessons Learned](#appendix-c)
30. [Appendix D: Registry Status Matrix](#appendix-d)
31. [Appendix E: Agent Prompts (Copy-Paste Ready)](#appendix-e)
...
<a id="phase-26"></a>
## Phase 26: Loop Dispatch Reliability, Salgskanal Auto-Matcher & Brreg Registration Wiring

**Duration:** 2026-07-06 → 2026-07-09 (3 days)
**Commits:** `44fbb43` (salgskanal), `4ba4a61`, `209a56f` (loop-dispatch), `66cd441` (brreg wiring), `d2882d0`, `87fba44` (fleet-auto-approve)
**Goal:** Fix a double-fire race in loop-dispatch triggered by boot lag, add auto-discovery of salgskanal (sales channel) categories, wire Brreg org verification into the registration flow, and improve GitHub Actions deployment reliability.

### 26.1 Salgskanal Auto-Matcher — Five Sales Channels, Zero Manual Curation (dev-request 2026-07-06-rfb-salgskanal-kategorier, slice 1)

Adds automated categorization of producers into five Norwegian sales channels, reducing manual tagging burden and enabling category-based discovery.

**New schema** (`src/database/init.ts`):

```sql
CREATE TABLE IF NOT EXISTS salgskanal_categories (
  id              TEXT PRIMARY KEY,
  slug            TEXT NOT NULL UNIQUE,         -- 'selvplukk', 'hjemlevering', etc.
  name_no         TEXT NOT NULL,                -- Norsk kategorinavn
...
CREATE TABLE IF NOT EXISTS agent_salgskanal (
  id              TEXT PRIMARY KEY,
  agent_id        TEXT NOT NULL REFERENCES agents(id) ON DELETE CASCADE,
  salgskanal_id   TEXT NOT NULL REFERENCES salgskanal_categories(id),
  source          TEXT NOT NULL DEFAULT 'auto', -- 'auto' or 'manual'
  matched_by      TEXT,                         -- Which matcher fired this tag
  created_at      TEXT NOT NULL,
  updated_at      TEXT NOT NULL
);

CREATE INDEX idx_agent_salgskanal ON agent_salgskanal(agent_id, salgskanal_id);
```

**Five channels** matched by `src/services/salgskanal-matcher.ts`:

1. **Selvplukk** (Pick Your Own) — requires "selv" + "plukk" in name/description/tags
2. **Hjemlevering** (Home Delivery) — specific phrasing like "levering hjem" or "delivery to home"
3. **Gårdsbutikk** (Farm Shop) — requires "gård" + ("butikk"/"shop"/"utsalg")
4. **Gårdskafé/Servering** (Farm Café) — strong signal for café/restaurant on-site (high bar: ≥2 weak signals or 1 strong signal like "kafé")
5. **REKO-ring** — requires both "reko" AND "ring" (not bare "reko" mentions)

**Matcher logic** (`src/services/salgskanal-matcher.ts`):

```typescript
export async function matchProducerSalgskanal(agent: Agent): Promise<string[]> {
  // Returns array of matched salgskanal slugs (0–5 items)
  // Scans: agent name, description, tags, skills
  // Applies precision-tuned regex/keyword logic per channel
  // Absent beats wrong — false positives excluded via compound conditions
}

// Used by:
// - POST /admin/salgskanal/sync (on-demand)
// - Daily scheduler tick at 05:00 UTC (mirroring debio-sync)
// - Both idempotent: overwrites 'auto' rows, preserves 'manual' overrides
```

**Test coverage:** 39 cases in `src/services/salgskanal-matcher.test.ts` covering:
- Dev-request's own known-true spot-probes
- Two named false-positive traps (e.g., a café mention nearby shouldn't tag a producer as having on-site café)

**Out of scope for this slice:** Public category landing pages, homepage discoverability, AI-discovery/MCP surface (planned for Phase 27).

**Commit:** `44fbb43` (2026-07-08)

---

### 26.2 Loop Dispatch Double-Fire Race Fix — Boot-Lag Deduplication (dev-request 2026-07-08-loop-dispatch-fire-marker-dedup)

Fixes a race condition where `/admin/loop-dispatch` could fire the same agent twice within one minute.

**Root cause:** When a Cloud Routine wakes up, it takes 30–90 seconds to boot before it POSTs its own `started_at` envelope to the run-ledger. If a second dispatch request arrives during that window, the dispatcher sees no fresh run yet (because the booting routine hasn't checked in) and fires the same agent again.

**Observed live:** Platform-orchestrator fired twice within the same minute on 2026-07-08 at 12:0x, 17:10, and 18:18 CEST.

**Solution:** Immediately after a successful `fireRoutine()` call, record a synthetic `firemarker-<agent-id>` run with:
- `finished_at == started_at == fire-time` (zero duration)
- `next_suggested: []` (can never wake itself)
- Attributed to the real agent name (so cooldown logic sees it)

The existing cooldown in `computeWakeList()` now finds the agent as "just ran" with ~0 lag instead of 30–90s, preventing the second fire.

**Integration:**

```typescript
// src/routes/admin-loop-dispatch.ts
async function planNow(): Promise<WakeList> {
  // ... existing logic ...
  
  const shouldFire = computeWakeList(recentRuns, ...);
  if (shouldFire.includes(agentName)) {
    const fireResult = await fireRoutine(agentName);
    // NEW: immediately record fire-marker
    await ledger.insertRun({
      run_id: `firemarker-${Date.now()}`,
      agent: agentName,
      started_at: fireTime,
      finished_at: fireTime,
      next_suggested: [],
      claims: []
    });
  }
...
**Side-effect filtering:** Fire-markers must be excluded from downstream consumers to avoid double-counting:
- `listPendingVerification()` — exclude `run_id LIKE 'firemarker-%'`
- `listStaleRuns()` — exclude firemarker rows
- `summariseRuns()` — exclude from activity rollup
- **Deliberately left unfiltered:** `listRecentRuns()` (dispatcher's own cooldown needs to see markers)

**Tests added:** Regression case in `src/tests/test.ts` verifies that a marker recorded 0 minutes ago is caught by cooldown immediately.

**Commits:** 
- `4ba4a61` (2026-07-08) — fire-marker insertion + run-ledger filtering
- Follow-up: `admin-loop-heartbeat.ts` filtering added (see below)

---

### 26.3 Loop Dispatch 24/7 Active Window — Env-Overridable Default (dev-request 2026-07-08-spine-24-7-active-window)

Changes `/admin/loop-dispatch` to send dispatches 24 hours a day by default (instead of 5–21 UTC hardcoded window).

**Previous behavior:** `computeWakeList()` had hardcoded `activeStartUTC=5, activeEndUTC=21` (daytime-only), and the dispatcher wasn't passing this to the function, so it always used the hardcoded window.

**New behavior:** `planNow()` now computes and sends explicit `activeStartUTC`/`activeEndUTC` to `computeWakeList()`:

```typescript
// Resolved from env, with fallback to 0/24 (24/7)
const activeStart = resolveActiveWindowHour(
  process.env.DISPATCH_ACTIVE_START_UTC,
  0 /* 24/7 default */
);
const activeEnd = resolveActiveWindowHour(
  process.env.DISPATCH_ACTIVE_END_UTC,
  24 /* 24/7 default */
);

const wakeList = computeWakeList(recentRuns, {
  activeStartUTC: activeStart,
  activeEndUTC: activeEnd,
  // ... other params unchanged ...
});
```

**Env override** (rollback without deploy):

```bash
# Restore 5–21 UTC window
DISPATCH_ACTIVE_START_UTC=5 DISPATCH_ACTIVE_END_UTC=21 npm start
```

**Rate-limit unchanged:** Cooldown (25min), maxWakes (4), freshness window (12min) are untouched — they prevent runaway spawning. The queue self-stops once `next_suggested[]` is empty.

**Helper function** (`src/services/loop-dispatch.ts`):

```typescript
export function resolveActiveWindowHour(
  envValue: string | undefined,
  fallback: number
): number {
  // Handles: unset, empty string, whitespace, non-numeric, out-of-range
  // Clamps result to [0, 24]
...
**Tests added:** Unit tests covering all env-parsing edge cases (empty string, non-numeric, out-of-range clamping).

**Commits:** 
- `209a56f` (2026-07-09) — 24/7 default + env-override path + helper tests
- 2026-07-08: Daniel confirmed in dev-request

---

### 26.4 Brreg Org Verification Wired into Registration (dev-request 2026-06-30-brreg-verification-gate, slice 2)

Completes the integration of Brreg verification into the agent registration flow for both RFB producers and experiences.

**Previous state (Phase 25 slice 1):** Brreg lookup service (`src/services/brreg-client.ts`) was built but not wired to any endpoint.

**New flow** (`src/routes/admin-agents.ts` / `POST /admin/agents/register`):

```typescript
// 1. Receive producer registration with optional org_nr
POST /admin/agents/register
  Body: { name, category, org_nr?: "123456789", ... }

// 2. If org_nr provided: verify against Brreg
if (body.org_nr) {
  const verification = await brregClient.verifyOrgNumber(body.org_nr);
  if (!verification || !verification.active) {
    return res.status(400).json({
      error: "invalid_org_nr",
      reason: "Organization not found or inactive in Brreg"
    });
  }
  // Store verification result
  agent.org_nr = org_nr;
  agent.brreg_verified = verification.active;
  agent.brreg_flag = "verified";
  agent.brreg_checked_at = new Date().toISOString();
}
...
**Test coverage** (363 new test lines in `src/routes/admin-agents.test.ts`):
- Valid org number → verified ✓
- Inactive org → rejected ✓
- Invalid org number → rejected ✓
- No org_nr provided → accepted (optional field) ✓
- Org mismatch vs. producer name → flagged but registered (allows manual override)

**Applies to both verticals:**
- RFB producers (`/admin/agents/register`)
- Experiences producers (same endpoint, shared logic)

**Schema fields** (already added in Phase 25):
- `agents.org_nr` — producer's claimed org number
- `agents.brreg_verified` — boolean (true if active in Brreg)
- `agents.brreg_flag` — "verified" | "invalid" | "mismatch" | null
- `agents.brreg_checked_at` — ISO 8601 timestamp of last check

**Follow-up phases:**
- Phase 27: Expose Brreg verification status in admin dashboard and public /agent/:id
- Phase 28: Use brreg_verified flag to gate outreach campaigns

**Commit:** `66cd441` (2026-07-08) — PR #162, 570 insertions

---

### 26.5 Fleet Auto-Approve Improvements — Explicit Deployment Trigger + Direct-Merge Fallback

Two fixes to the GitHub Actions deployment automation to improve reliability.

**Fix 1: Explicit fly-deploy.yml Trigger** (PR #178, `d2882d0`)

**Problem:** GitHub suppresses workflow triggers (push, pull_request, etc.) generated by the default GITHUB_TOKEN to prevent infinite loops. `fleet-auto-approve.yml` runs under this default token and merges PRs, but the merge silently fails to trigger `fly-deploy.yml`'s `on: push` event.

**Observed:** PRs merged by bot, no deployment check-runs 9 minutes later, `/health` stayed on prior SHA until manual `workflow_dispatch`.

**Solution:** After merge confirmation, explicitly call `actions.createWorkflowDispatch()` for `fly-deploy.yml` on `ref: main`. The `workflow_dispatch` event is NOT suppressed by GitHub's anti-recursion rule.

```typescript
// src/.github/workflows/fleet-auto-approve.yml
if (merged) {
...
**Fix 2: Direct-Merge First, Fallback on Demand** (PR #178)

Reorders merge attempts:
1. **Try direct squash-merge first** — the gate already confirmed all checks green, so this is the common case, fast path
2. **Fallback to enablePullRequestAutoMerge** — only if direct merge isn't immediately possible yet (~12s poll)

This reduces latency and avoids unnecessary API calls.

**Commit:** `d2882d0` (2026-07-08) — PR #178

**Follow-up fix** (`87fba44`, PR #175): When a PR is already clean/mergeable, GitHub's auto-merge API sometimes returns "already merged" error. New code accepts this as success and skips the retry poll.

---

### 26.6 Loop-Heartbeat Stall Detection — Exclude Fire-Markers from Health Check

Independent review finding on Phase 26.2: `/admin/loop-heartbeat`'s stall-watchdog could be silenced by fire-markers alone, masking real agent failures.

**Problem:** `computeLoopHealth()` feeds `listRecentRuns()` (deliberately unfiltered to serve dispatcher) into `DEFAULT_WATCHERS` (which includes platform-verifier as a legitimate wake target). A fire-marker only proves a `/fire` call was made, not that the agent completed a real cycle, so relying on markers as heartbeats could mask failure.

**Solution:** Filter firemarker rows at the heartbeat call site only:

```typescript
// src/routes/admin-loop-heartbeat.ts
async function readHealth(): Promise<LoopHealth> {
  const recentRuns = await ledger.listRecentRuns();
  // Filter: exclude fire-markers before checking stall conditions
  const realRuns = recentRuns.filter(r => !r.run_id.startsWith('firemarker-'));
  return computeLoopHealth(realRuns, ...);
}
```

**Preserves existing behavior:**
- `listRecentRuns()` stays unfiltered (dispatcher's cooldown needs markers)
- `/admin/runs` (raw audit listing) stays unfiltered (users can filter if needed)
- Only heartbeat/stall-detection filters at point of use

**Commit:** Part of `4ba4a61` (2026-07-08) — PR #179, review round 3

---

### 26.7 Gotchas & Lessons Learned — Phase 26 Additions

**C.132: Salgskanal matcher thresholds are tightly tuned for Norwegian phrasing**  
The regex/keyword logic is calibrated for Norwegian text and false-positive traps identified in dev-request. If scaling to other languages, re-test the matcher against a corpus of real producer names/descriptions to avoid spurious tags. The 39-case test suite covers only RFB's known-true and known-false examples.

**C.133: Fire-marker rows are synthetic and must not escape filters**  
Any new query on run-ledger that summarizes or counts runs (KPIs, dashboards, reports) should explicitly exclude `run_id LIKE 'firemarker-%'`. These rows exist only to serve the dispatcher's cooldown logic; they don't represent real agent work and will distort metrics if leaked. The pattern: `WHERE run_id NOT LIKE 'firemarker-%'`.

**C.134: 24/7 dispatch window still respects rate-limits**  
Changing `activeStart/activeEnd` to 0/24 does NOT cause runaway firing — the cooldown window (25min) and maxWakes (4) still apply per agent. If you find the system over-firing after enabling 24/7, check that `next_suggested[]` is being populated correctly (agents that run stale will remain in the queue, and the cap of 4 concurrent wakes will prevent flooding).

**C.135: Brreg verification is best-effort, not authoritative**  
Brreg can return 404 for very new orgs (lag in their own data) or 5xx errors (maintenance). The registration endpoint treats "not found" as invalid and rejects the producer; if manual override is needed, add an admin endpoint to backfill/override the verification result later. Don't gate critical flows on Brreg being 100% available.

**C.136: Direct-merge-first strategy reduces auto-approve latency but requires pre-merge CI green**  
The new squash-merge first path assumes all status checks passed (the gate confirms this). If CI is slow or flaky, the direct-merge path might still fail due to a race (PR was eligible 10s ago, but a new commit pushed in the last second). The fallback to `enablePullRequestAutoMerge` + poll handles this, but it's slower. Future optimization: add a ~5s re-check of CI before attempting direct merge.
